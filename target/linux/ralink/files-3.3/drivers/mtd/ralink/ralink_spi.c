...
#endif
{
	...
	struct mtd_partition *linux_part,*kernel_part,*rootfs_part;
...
#endif
	partitions_num = sizeof(rt2880_partitions)/sizeof(struct mtd_partition);
	
	for(i = 0; i < partitions_num; i++)
	{
	 if(rt2880_partitions[i].name == "firmware")
	 {
	   linux_part = &rt2880_partitions[i];
	 }
	 else if(rt2880_partitions[i].name == "kernel")
	 {
	   kernel_part = &rt2880_partitions[i];
	 } 
	 else if(rt2880_partitions[i].name == "rootfs")
	 {
	   rootfs_part = &rt2880_partitions[i];
	 }
	};
#define SKIP_KERNEL_SIZE 0xB0000
#define SKIP_BLOCK_SIZE	(4*1024)  /* 4KiB */
	
	offset=linux_part->offset;
	for (offset += SKIP_KERNEL_SIZE; offset < linux_part->size; offset += SKIP_BLOCK_SIZE)
	{
		/* Find the squashfs. */
		memset(buf, 0xa5, sizeof(buf));
		if (ramtd_read(NULL, offset, sizeof(buf), &len, buf) || len != sizeof(buf)) continue;
//		if (memcmp(buf, PACKIMG_TAG, 12)) continue;
		/* We found the tag, check for the supported file system. */
		squashfsb = (struct squashfs_super_block *)buf;
		if (squashfsb->s_magic == SQUASHFS_MAGIC)
		{
			printk(KERN_NOTICE "Squashfs filesystem found at offset 0x%x,adjustment mtd partition.\n",  offset);
			kernel_part->size = offset - linux_part->offset;
			kernel_part->mask_flags = MTD_WRITEABLE;
			rootfs_part->offset = offset;
			rootfs_part->size = flash->mtd.size - rootfs_part->offset;
			rootfs_part->mask_flags = !MTD_WRITEABLE;
			linux_part->size = flash->mtd.size - linux_part->offset;
			break;
			//return rt2880_partitions;
		}
	}
...
#else
	mtd_device_register(&flash->mtd, rt2880_partitions, ARRAY_SIZE(rt2880_partitions));
	return &flash->mtd;
#endif
}
